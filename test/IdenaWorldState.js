...
contract("IdenaWorldState", (accounts) => {
  ...
  describe.only("> verify", async () => {
    for (const [i, d] of idenaData.verify.entries()) {
      it(`check verify (${i + 1}): keys=${d.keys}, message=${d.message}`, async () => {
        // bad message
        await mock.verify.estimateGas(d.apk1, d.apk2, d.message+"bad", d.signature).should.be.rejectedWith(h.EVMRevert)
        // bad apk1
        ...
        // bad apk2
        let badApk2 = [d.apk2[0], d.apk2[1], d.apk2[2], "0x4444444444444444444444444444444444444444444444444444444444444444"]
  ...
  describe("> initialize", async () => {
    ...
    const identities = [accounts[1],
      accounts[2]
    ]
    const pubkeys = [[new BN(h.randHex(64), 16),
      new BN(h.randHex(64), 16)
    ],
      [new BN(h.randHex(64), 16),
        new BN(h.randHex(64), 16)
      ]
