...
* **population**: the number of validated identities. Because we use a special method to maintain the state to save gas, this number have to be used instead of the `identities.length`.
...
* `function identities() public view returns (address[] memory)`: Get the list of all encrypted users validated in idena.
* `function identityByIndex(uint256 i) public view returns (address)`: Query identity address by index.
* `function isIdentity(address addr) public view returns (bool)`: Check if the specific identity is validated in idena.
...
We can found gas costs of all the operations in the output as a reference.
...
* Split `update()` function to support multi-transactions calling.
  - Because an transaction may need more gas than block gas limit if there are to identities to add.
